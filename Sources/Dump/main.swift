...
let image: CGImage = {
    if let imagePath,
        let img = ImageLibrary.load(URL(fileURLWithPath: imagePath), maxPixel: 2560)
    {
        return img
    }
...
// A fixed seed makes a render reproducible, which is what lets the test suite
// compare two builds byte for byte.
let seed = ProcessInfo.processInfo.environment["WINDFLOW_SEED"].flatMap { UInt32($0) }
let skipPresent = ProcessInfo.processInfo.environment["WINDFLOW_NO_PRESENT"] != nil
let prepared = PreparedImage.prepare(
    image: image, canvasWidth: width, canvasHeight: height,
    coverW: coverW, coverH: coverH, settings: settings, seed: seed)
let sim = Simulation.begin(prepared, canvas: canvas, settings: settings, preview: false)
print(
    String(
        format: "canvas %dx%d  field %dx%d  regions %d  setup %.0f ms",
    ...
    if captureIndex < captureAt.count && t >= captureAt[captureIndex] {
        ...
        if ProcessInfo.processInfo.environment["WINDFLOW_MASK"] != nil {
            if let mask = canvas.debugCoverageImage(),
                let png = NSBitmapImageRep(cgImage: mask).representation(
            ...
            print(
                "  coverage deciles: "
                    + bins.map { String(format: "%.1f%%", Double($0) * 100 / Double(total)) }
                    .joined(separator: " "))
        }
        captureIndex += 1
    }
...
print(String(format: "paint checksum %016llx", canvas.debugChecksum()))
print(
    String(
        format: "sim %.2f ms/frame, present %.2f ms/frame over %d frames",
